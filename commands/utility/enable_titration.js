	...
	data: new SlashCommandBuilder()
		...
		.addIntegerOption(option =>
			option
				...
				.setDescription('The number of users to attempt to grant the approval role (default 10 users)')
				.setRequired(false))
		.addIntegerOption(option =>
			option
				.setName('period')
				.setDescription('The time interval between titrations in seconds (default 1h)')
				.setRequired(false)),
	async execute(interaction, bot_instance) {

		let desired_count = interaction.options.getInteger('desired_count') ?? bot_instance.max_ingresses_per_period;
		let period = interaction.options.getInteger('period') ?? bot_instance.wave_period_seconds;
